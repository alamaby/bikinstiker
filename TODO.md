...
- [x] **SH1** Migrasi `20260830000001_security_hardening.sql`: [1] 21 fungsi internal-only → REVOKE anon+authenticated+PUBLIC + GRANT service_role; [2] 35 fungsi client → REVOKE anon; [3] 11 analytics views → REVOKE SELECT dari anon+authenticated; [4] search_path pin ×2 trigger fn. **Done (2026-08-30)** — pending deploy.
- [x] **SH2a (verifikasi pasca-push #1, 2026-08-30)** curl anon ke produksi: 4 fungsi internal (refund_failed_sticker, acquire_generation_lock, admin_grant_credits, expire_old_credits) → **`42501 permission denied`** ✅. **TEMUAN**: 35 fungsi client + views MASIH bisa dipanggil anon — grant efektifnya berasal dari **PUBLIC** (default privileges), `REVOKE FROM anon` saja no-op; bukti: `claim_daily_checkin`/`get_pack_detail` tereksekusi (error internal P0001), `search_showcase_listings` 200. Fix: migrasi follow-up `20260830000002_security_hardening_public_closure.sql` (REVOKE anon+PUBLIC, GRANT authenticated+service_role; views ditutup dari PUBLIC juga).
- [x] **SH2b (verifikasi pasca-push #2, 2026-08-30 — agent via curl anon produksi)** Setelah `20260830000002` di-push: **56/56 fungsi DENIED** (14 internal 42501/PGRST202 + 28 client ber-argumen 42501 + 9 zero-arg 42501 — termasuk 3 yang sebelumnya terbukti tereksekusi) dan **11/11 views DENIED** (`permission denied`). Tabel kontrol `sticker_presets?select=id` tetap 200 `[]` (deny-all RLS utuh). Sisa verifikasi manual owner: **smoke app penuh** dengan akun guest + registered (guest generate, check-in, surprise me, packs CRUD, showcase browse+purchase, legal gate, share link) — memastikan role `authenticated` tetap kebagian GRANT eksplisit.
- [ ] **SH2c** Re-run advisor: `security_definer_view` & `anon_security_definer_function_executable` harus kosong. Catatan: `authenticated_security_definer_function_executable` **tetap muncul untuk ±35 fungsi client — by design** (pola repo: RPC client memang SECURITY DEFINER + auth.uid() internal; kini juga eksplisit GRANT authenticated).
- [ ] **SH3 (backlog)** Audit body `rls_auto_enable()` via SQL Editor (tidak ada di migrasi — dibuat manual; drift Database-as-Code) → port ke migrasi atau drop. Perf opsional: 18 policy initplan → `(select auth.uid())`; index FK tabel besar; pindahkan pg_trgm dari public.
- [ ] **SH4 (disiplin)** Setiap migrasi baru yang `CREATE OR REPLACE` fungsi dari daftar SH1 WAJIB mengulang REVOKE terkait (default privileges platform re-grant otomatis) — catatan juga di header migrasi hardening.
