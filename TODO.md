...
- [ ] **SH2 (manual, owner)** `supabase db push` lalu verifikasi:
  - SQL Editor: `select has_function_privilege('anon','public.acquire_generation_lock(uuid,uuid,timestamptz)','EXECUTE');` → **false** (cek juga `refund_failed_sticker(uuid)`, `admin_grant_credits(...)`); `select has_table_privilege('anon','public.user_engagement_metrics','SELECT');` → **false**.
  - curl anon: POST `/rest/v1/rpc/refund_failed_sticker` + `/rpc/acquire_generation_lock` dengan apikey anon → harus **permission denied/403**; GET `/rest/v1/user_engagement_metrics?select=*` → kosong/error.
  - Smoke app penuh: guest generate, check-in, surprise me, packs CRUD, showcase browse+purchase, legal gate, share link.
  - Re-run advisor: temuan `security_definer_view` harus hilang; `anon_security_definer_function_executable` harus kosong. Catatan: `authenticated_security_definer_function_executable` **tetap muncul untuk ±35 fungsi client — by design** (pola repo: RPC client memang SECURITY DEFINER + auth.uid() internal; jangan dijadikan target revoke authenticated).
