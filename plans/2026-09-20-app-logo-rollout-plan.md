...
- [x] **T2d — Version bump (patch + build)**
  - File: `pubspec.yaml:4` currently `version: 0.26.9+91`.
  - Change to `version: 0.26.10+92` (minor stays, patch 9→10, build 91→92). Nothing else in pubspec changes in this task.
  - Verify: `rg -n "^version:" pubspec.yaml` prints `0.26.10+92`.
...
- 2026-09-20 19:07:00 — T4 skipped: sibling repo `bikin-stiker-landing-page` not present locally at `C:\Works\github.com\alamaby\`; owner to execute manually.
...
- [x] **T2d — Version bump 0.26.10+92**
- [x] **T2e — In-app logo (PNG + pubspec + auth header)**
- [x] **T3 — Verify app (analyze 0 issues, 208 tests pass, APK+AAB green, draft excluded)**
- [ ] **T4 — Landing sibling** (repo not local; manual)
...
- T0 note: programmatic flat via PIL distance-threshold (bg `(2,107,186)` → target `(0,114,178)`, threshold 20) produced usable masters. Manual designer review recommended for sparkle/outline fidelity before handoff to Play/iOS submission.
