...
void main() {
  ...
  blocTest<AuthBloc, AuthBlocState>(
    ...
    build: () {
      when(() => repo.sendEmailOtp(email: any(named: 'email'))).thenAnswer(
        (_) async {},
      );
  ...
  blocTest<AuthBloc, AuthBlocState>(
    ...
    build: () {
      when(() => repo.sendEmailOtp(email: any(named: 'email')))
  ...
  blocTest<AuthBloc, AuthBlocState>(
    'guest-wall OTP send failure stays guest with error',
    build: () {
      when(() => repo.sendEmailOtp(email: any(named: 'email')))
          .thenThrow(const AuthFailure('Signups not allowed for otp'));
      return AuthBloc(repo);
  ...
  blocTest<AuthBloc, AuthBlocState>(
    ...
    build: () {
      when(() => repo.verifyEmailOtp(email: any(named: 'email'), token: any(named: 'token'))).thenAnswer(
        (_) async {},
      );
  ...
  blocTest<AuthBloc, AuthBlocState>(
    ...
    build: () {
      when(() => repo.verifyEmailOtp(email: any(named: 'email'), token: any(named: 'token')))
  ...
  blocTest<AuthBloc, AuthBlocState>(
    ...
    seed: () => AuthBlocState(status: AuthStatus.guest, user: guestUser),
    act: (bloc) =>
        bloc.add(const AuthOtpVerifyRequested('a@mail.com', '12345678', isGuestAuthWall: true)),
    expect: () => [
      AuthBlocState(status: AuthStatus.submitting, user: guestUser),
      AuthBlocState(
        status: AuthStatus.guest,
