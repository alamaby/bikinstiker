...
void main() {
  group('DailyCheckinCard renders day boxes', () {
    ...
    testWidgets('day 1 shows checkmark after checking in today',
        (tester) async {
      await tester.pumpWidget(
        buildCard(
    ...
    testWidgets('day 1 shows themed marker when not checked in yet',
        (tester) async {
      await tester.pumpWidget(
        buildCard(
          streak: DailyCheckinStreak.fromJson({
      ...
      expect(find.text('\u{1F305}'), findsOneWidget,
          reason: 'Day 1 should show sunrise emoji when not completed');
    });

    testWidgets('shows "Cycle complete!" text when cooldown active',
        (tester) async {
      await tester.pumpWidget(
        buildCard(
          streak: DailyCheckinStreak.fromJson({
    ...
    testWidgets('shows "Start new cycle" button after cooldown',
        (tester) async {
      await tester.pumpWidget(
        buildCard(
          streak: DailyCheckinStreak.fromJson({
    ...
    testWidgets('all 7 days show checkmark when cycle completed',
        (tester) async {
      await tester.pumpWidget(
        buildCard(
          streak: DailyCheckinStreak.fromJson({
